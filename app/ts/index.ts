let firstPlayerButton = document.querySelector(
  "#firstPlayerButton"
) as HTMLButtonElement;
let secondPlayerButton = document.querySelector(
  "#secondPlayerButton"
) as HTMLButtonElement;
...
function player1Start() {
  ...
  if (showScoreP1) {
    ...
    if (totalScore) {
      totalScore.textContent = String(
        player1.scores.reduce((acc, curr) => acc + curr, 0)
      );
    }
  }
  checkTotalScore(
    player1.scores.reduce((a: number, b: number) => a + b, 0),
    player2.scores.reduce((a: number, b: number) => a + b, 0)
  );
}
function player2Start() {
  ...
  if (totalScoreP2) {
    totalScoreP2.textContent = String(
      player2.scores.reduce((acc, curr) => acc + curr, 0)
    );
  }
  checkTotalScore(
    player1.scores.reduce((a: number, b: number) => a + b, 0),
